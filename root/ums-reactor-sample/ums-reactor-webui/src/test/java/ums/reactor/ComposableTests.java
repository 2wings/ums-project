...
public class ComposableTests {
    ...
    @Test
    public void testBasicComposable() {
    ...
    public void testComposeFromSingleValue() throws InterruptedException {
        Stream<String> stream = Streams.defer("Hello World!").get();
        Stream<String> s = stream.map(new Function<String, String>() {
            @Override
    ...
    public void testComposeFromMultipleValues() throws InterruptedException {
        ...
        Stream<Integer> s = stream.map(STRING_2_INTEGER).map(new Function<Integer, Integer>() {
            int sum = 0;
            @Override
    ...
    public void testComposeFromMultipleFilteredValues() throws InterruptedException {
        Stream<String> stream = Streams.defer(Arrays.asList("1", "2", "3", "4", "5")).get();
        Stream<Integer> s = stream.map(STRING_2_INTEGER).filter(new Predicate<Integer>() {
    ...
    public void testComposedErrorHandlingWithMultipleValues() throws InterruptedException {
        Stream<String> stream = Streams.defer(Arrays.asList("1", "2", "3", "4", "5")).env(env).dispatcher("eventLoop")
                .get();
    ...
    public void testReduce() throws InterruptedException {
        Stream<String> stream = Streams.defer(Arrays.asList("1", "2", "3", "4", "5")).get();
        Stream<Integer> s = stream.map(STRING_2_INTEGER).reduce(new Function<Tuple2<Integer, Integer>, Integer>() {
    ...
    public void testRelaysEventsToReactor() throws InterruptedException {
        ...
        Stream<String> stream = Streams.defer(Arrays.asList("1", "2", "3", "4", "5")).get();
        Stream<Integer> s = stream.map(STRING_2_INTEGER).consume(key.getObject(), r);
    ...
    public void testStreamBatchesResults() {
        Stream<String> stream = Streams.defer(Arrays.asList("1", "2", "3", "4", "5")).get();
        Stream<List<Integer>> s = stream.map(STRING_2_INTEGER).collect();
    ...
    public void mapManyFlushesAllValuesThoroughly() throws InterruptedException {
        ...
        // }));
        //
        // tasks.consume(new Consumer<Integer>() {
        //
        // @Override
